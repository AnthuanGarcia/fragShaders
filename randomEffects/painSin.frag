...
void main() {
    ...
    col = mix(
        ...
        plot(abs(cc.x), 0.01) +
        plot(abs(cc.y), 0.01) + 
        plot(abs(polarGrid.y), 0.04)) * cropGrid
    );
    ...
    col = mix(
        col,
        vec3(0.25),
        max(plot(grid.x, 0.025), plot(grid.y, 0.025)) * cropMid
    );
    ...
    col = mix(
        ...
        //plot(abs(s.y), 0.01) * cropMid
        GLOW(0.0085, abs(s.y), 0.7) * cropMid
    );
    ...
    col = mix(
        ...
        //plot( abs( CIRCLE(R, cc) ), 0.01) +
        //plot( CIRCLE(0.03, cc), 0.0 )
        GLOW(0.02, abs( CIRCLE(R, cc) ), 0.8)
    );

    // Hypothenuse = tangent = radius
    col = mix(
        col,
        vec3(1),
        //plot( abs(rcc.y), 0.01 ) * len * side
