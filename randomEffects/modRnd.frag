...
float scene(vec3 p) {
	...
	float noise = 5.*noiseDist(p - 0.25*u_time);

	return sdPlane(p, vec3(0, 1, 0), 1.5) - noise;
...
vec3 shade(vec3 ro, vec3 pos) {
	...
	float dirCol = (1.0 + dot(n, lightDir)) * 0.5;
	...
	//vec3 ambient = vec3(0.5, 0.25, 0.0);
